...
> 当前 `v1.0.15` 为测试中的过渡版本。
> QQ 扫码绑定已临时回退到旧版风格链路做回归验证，暂不保证完全可用。
> 如果你依赖稳定使用，建议先在测试环境验证；如果你愿意协助测试，欢迎优先反馈日志。
...
- 修复 QQ 绑定成功后拉取角色资料时，接口直接返回正常中文 `msg` 会触发 `latin1` 重编码崩溃的问题；现在仅在明显属于旧版乱码时才尝试按 `gbk` 还原
- 补充角色资料 `msg` 的双路径回归测试：正常中文保持原样，旧版 `latin1 -> gbk` 乱码仍可正确解码
- 新增测试版标识：插件元信息、README 与更新日志均明确标注“当前版本仍在测试中，暂不保证完全可用”
- QQ 扫码绑定默认临时回退到更接近早期可用实现的旧版风格链路，使用独立真实 CookieJar session 跑完整个登录与兑换流程
- 近期多轮尝试的 `oauth2.0/show` / 新授权上下文代码暂时保留为对照实现，但不再作为默认主链路
- QQ 默认短链路在换取令牌前会先回访一次官方 `authorize/show` 页面，补齐 `graph.qq.com` 侧授权上下文后再提交授权表单，降低扫码成功后仍被打回 `oauth2.0/show` 的概率
- 默认屏蔽近几版持续失败的 `oauth2.0/show` 授权页链路，QQ 绑定改回优先走更短的 legacy `authorize -> code -> qcCodeToOpenId` 兑换流程
- 仅在显式开关或测试场景下才会进入 `oauth2.0/show` 授权页链路；若该路径再次被 QQ 打回展示页，会自动回退到 legacy 直连兑换流程
- QQ 换取令牌阶段仍保留官方授权页上下文代码，便于后续继续对照官网行为排查问题
- QQ 扫码绑定入口改为先走三角洲官网同款的 QQ Connect authorize 页面，再提取内层 `xlogin`，与官网当前登录层级保持一致
